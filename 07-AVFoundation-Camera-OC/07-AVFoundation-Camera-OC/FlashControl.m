...
-(void) setupView{
    ...
    _expandWidth = ICON_WIDTH + (BUTTON_WIDTH * self.labels.count);
...
-(NSArray *)buildLabels:(NSArray *)labelStrings{
    ...
    for (NSString *string in labelStrings) {
        CGRect frame = CGRectMake(x, self.midY, BUTTON_HEIGHT, BUTTON_HEIGHT);
...
-(void)selectMode:(id)sender forEvent:(UIEvent *)event{
    if (!self.expanded) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self.frameWidth = self.expandWidth;
...
-(void)selSelectedIndex:(NSUInteger) selectedIndex{
